...
const Blog = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const response = await fetch('https://ecom-server-production-7e4d.up.railway.app/api/blogposts');
...
const BlogCard = ({ blog }) => {
  const [imgSrc, setImgSrc] = useState(
    blog.image 
      ? `https://ecom-server-production-7e4d.up.railway.app/uploads/${blog.image}`
